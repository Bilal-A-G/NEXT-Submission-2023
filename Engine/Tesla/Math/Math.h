...
namespace TESLA
{
    ...
    struct Matrix4x4
    {
        ...
        static Matrix4x4 PointAt(Vector3 position, Vector3 target, Vector3 up)
        {
            ...
            Vector3 localUp = (up - forward * TESLA::Vector3::Dot(up, forward)).Normalize();
            Vector3 right = TESLA::Vector3::Cross(localUp, forward);
        ...
        static Matrix4x4 LookAt(Matrix4x4 pointAt)
        {
            ...
            {
                {pointAt.r0.x, pointAt.r0.y, pointAt.r0.z, -TESLA::Vector3::Dot(r0Vector, r3Vector)},
                {pointAt.r1.x, pointAt.r1.y, pointAt.r1.z, -TESLA::Vector3::Dot(r1Vector, r3Vector)},
                {pointAt.r2.x, pointAt.r2.y, pointAt.r2.z, -TESLA::Vector3::Dot(r2Vector, r3Vector)},
                {0, 0, 0, 1.0f},
            };
    ...
    inline Vector3 IntersectPlane(Vector3 planePos, Vector3 planeNormal, Vector3 lineStart, Vector3 lineEnd)
    {
        ...
        const float planeD = -TESLA::Vector3::Dot(normal, planePos);
        const float aD = TESLA::Vector3::Dot(lineStart, normal);
        const float bD = TESLA::Vector3::Dot(lineEnd, normal);
