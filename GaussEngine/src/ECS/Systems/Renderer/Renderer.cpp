...
{
    std::vector<Component*>& meshComponents = lookup.GetComponents(GAUSS_ENUMS::Mesh);
    std::vector<Component*>& transformComponents = lookup.GetComponents(GAUSS_ENUMS::Transform);
    
    Light* light = lookup.GetFirstValidComponent<Light>(GAUSS_ENUMS::Light);
    Transform* lightTransform = lookup.GetComponent<Transform>(GAUSS_ENUMS::Transform, light->m_entityId);

    if(!lightTransform)
        assert(false);
    
    Camera* camera = lookup.GetFirstValidComponent<Camera>(GAUSS_ENUMS::Camera);
    Transform* cameraTransform = lookup.GetComponent<Transform>(GAUSS_ENUMS::Transform, camera->m_entityId);
    ...
    for (Component* component : meshComponents)
    {
        ...
        std::vector<Face> faces = ProjectToWorld(model, mesh->faces);
        ...
        faces = ProjectToScreen(faces, camera->GetProjection());
